...
| Interactive onboarding | Evolve the static how-to-play screen into a playable tutorial or staged mechanic demos if players still need more teaching help | The current reference screen is intentionally concise and non-interactive | Needs evidence that the reference screen is not enough, plus a scope that does not bloat the fixed canvas UI |
| Campaign expansion | Expand the generated campaign beyond the shipped 20 levels once the tutorial and final-test pacing feel stable | The repo now ships a 20-level generated campaign with a 10-level mechanic tutorial and a 10-level final test | Needs more playtest feedback on long-form difficulty pacing before scaling the count again |
